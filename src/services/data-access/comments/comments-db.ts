import {type Comment, type CommentRecord} from "@domain/comment/types";
...
export default function makeCommentsDb({makeDatabase}: {
    ...
    return Object.freeze({
        async insert (comment: Comment): Promise<CommentRecord> {
            const db = makeDatabase();
            const source = comment.getSource();
            const query = `
                INSERT INTO public.comments (id, text, author, post_id, reply_to_id, published, modified_on, ip, browser, referer, hash)
        ...
        async findById(id: string): Promise<CommentRecord | undefined> {
            const db = makeDatabase();
        ...
        async findByAuthor(author: string): Promise<CommentRecord[]> {
            const db = makeDatabase();
        ...
        async findPublishedByPostId(postId: string): Promise<CommentRecord[]> {
            ...
            const query = `
                SELECT * FROM public.comments 
                WHERE post_id = $1 AND published = true AND deleted_at IS NULL 
                ORDER BY created_at ASC
            `;
            const values = [postId];
            const result = await db.query(query, values);
        ...
        async findAll(): Promise<CommentRecord[]> {
            const db = makeDatabase();
            const query = `SELECT * FROM public.comments WHERE deleted_at IS NULL ORDER BY created_at DESC`;
        ...
        async update(comment: Comment): Promise<CommentRecord> {
            ...
            const query = `
                ...
                SET text = $2, author = $3, post_id = $4, reply_to_id = $5, published = $6, 
                    ip = $7, browser = $8, referer = $9, hash = $10, modified_on = CURRENT_TIMESTAMP
        ...
        async findByHash(hash: string): Promise<CommentRecord | undefined> {
            const db = makeDatabase();
            const query = `SELECT * FROM public.comments WHERE hash = $1 AND deleted_at IS NULL`;
            const values = [hash];
            const result = await db.query(query, values);
            return result.rows[0];
        },
        ...
        async hardDelete(id: string): Promise<CommentRecord> {
            const db = makeDatabase();
            const query = `DELETE FROM public.comments WHERE id = $1 RETURNING *`;
            const values = [id];
            const result = await db.query(query, values);
            return result.rows[0];
        },
        ...
        async publish(id: string): Promise<CommentRecord> {
            ...
            const query = `
                UPDATE public.comments 
                SET published = true, modified_on = CURRENT_TIMESTAMP 
                WHERE id = $1 AND deleted_at IS NULL
                RETURNING *
            `;
            const values = [id];
        ...
        async unpublish(id: string): Promise<CommentRecord> {
            ...
            const query = `
                UPDATE public.comments 
                SET published = false, modified_on = CURRENT_TIMESTAMP 
                WHERE id = $1 AND deleted_at IS NULL
                RETURNING *
            `;
            const values = [id];
